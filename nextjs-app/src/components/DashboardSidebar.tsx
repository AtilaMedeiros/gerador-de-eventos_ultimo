...
// NavItem component outside of render
function NavItem({
    to,
    icon: Icon,
    label,
...
export function DashboardSidebar() {
    ...
    const isChildActive = useCallback(
        (path: string) => pathname?.startsWith(path) || false,
        [pathname],
    )
    ...
    return (
        <aside className="hidden w-72 flex-col border-r bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60 text-sidebar-foreground md:flex min-h-screen sticky top-0 h-screen overflow-y-auto z-40 shadow-subtle">
            <Link href="/area-do-produtor/inicio" className="p-6 border-b border-border/50 flex items-center gap-3 hover:opacity-80 transition-opacity">

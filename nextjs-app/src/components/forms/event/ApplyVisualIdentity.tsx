...
import { Palette, Check, ArrowLeft, Save, Eye, Rocket, X, Edit, Trash2 } from 'lucide-react'
import { toast } from 'sonner'
import { cn } from '@/lib/utils'
import { Dialog, DialogContent } from '@/components/ui/dialog'
import { EventPreview } from '@/components/EventPreview'
import VisualIdentityForm from '@/components/forms/theme/VisualIdentityForm'
...
export default function ApplyVisualIdentity({
    ...
    const { themes, deleteTheme } = useEventTheme()
    ...
    // Sync with Event and Default Theme
    useEffect(() => {
    ...
    const handleSave = async () => {
        ...
        await updateEvent(selectedEventId, { themeId: selectedThemeId, status: 'published' })
    ...
    const handleDraft = async () => {
        if (!selectedEventId) return
        await updateEvent(selectedEventId, { themeId: selectedThemeId, status: 'draft' })
        ...
        <div className={cn("w-full max-w-7xl mx-auto flex flex-col pt-6 px-6", isWizard ? "h-full" : "h-[calc(100vh-5rem)]")}>
            ...
            <div className="flex-1 grid grid-cols-1 lg:grid-cols-12 gap-6 overflow-y-auto pr-2 lg:pr-4 scrollbar-thin pb-24">
                ...
                <div className="lg:col-span-4 flex flex-col">
                    <Card className="flex flex-col shadow-md border bg-card rounded-xl overflow-hidden h-full">
                                    ...
                                    <div className="flex items-end justify-between mt-auto">
                                        ...
                                        <div className="flex items-center justify-end gap-1 -mr-1">
                                            <Button
                                                variant="ghost"
                                                size="icon"
                                                className="h-7 w-7 text-muted-foreground hover:text-primary z-20"
                                                onClick={(e) => {
                                                    e.stopPropagation()
                                            ...
                                            <Button
                                                variant="ghost"
                                                size="icon"
                                                className="h-7 w-7 text-muted-foreground hover:text-destructive z-20"
                                                onClick={(e) => {
                                                    e.stopPropagation()
                ...
                <div className="lg:col-span-8 flex flex-col bg-background/50 rounded-xl border shadow-sm p-1">
                    <div className="rounded-lg overflow-hidden relative bg-muted/10 h-full min-h-[500px]">
                        {currentTheme ? (
